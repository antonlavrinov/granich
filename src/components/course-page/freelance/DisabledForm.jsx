    ...
    <Form action="#" method="post">
      <ParticipationWarning>
        ...
        <ParticipationWarningText>
          Записаться можно только предварительно отправив мне своё портфолио для
          оценки вашей компетенции. После чего я вышлю вам ссылку для оплаты
          места
        </ParticipationWarningText>
      </ParticipationWarning>
      <FormFieldsWrapper>
        <FormContainer>
          <FormTags>
            {data.courseStream && (
              <FormTag important>{data.courseStream} поток</FormTag>
            )}
            {data.courseStartAndEnd && (
              <FormTag>{data.courseStartAndEnd}</FormTag>
            )}
            {data.courseTags.map((tag, idx) => {
              return <FormTag key={idx}>{tag}</FormTag>
            })}
          </FormTags>
          <FormInput type="text" placeholder="Имя" disabled={true} /> <br />
          <FormInput
            type="text"
            placeholder="Фамилия"
          ...
          {/* <FormInputLabel htmlFor="formParams[email]">чтобы выслать ссылку для входа</FormInputLabel>
                    <br /> */}
          <FormInputLabelMobile htmlFor="formParams[email]">
          ...
          <FormPolitikaLabel>
            ...
            <Link to="/privacy"> политики конфиденциальности</Link> <br />и 
      ...
      <FormFooterWrapper>
        <FormFooterContainer>
          <FormButtonBlock>
            <FormButton disabled={true} type="submit" id="button970916">
              Оплатить
            </FormButton>
            {data.coursePrice && (
              <FormPrice>
                <i>→ </i>
                {formatPrice(data.coursePrice)}
                <span>₽</span>
              </FormPrice>
            )}
          </FormButtonBlock>
