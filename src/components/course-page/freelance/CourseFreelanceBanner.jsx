...
const CourseFreelanceBanner = () => {
  const imageData = useStaticQuery(graphql`
    query bannerFreelancePodborkaImage {
      freelancePodborkaImageBanner: file(
        relativePath: { eq: "freelance/course-banner/course-banner.png" }
      ) {
        childImageSharp {
      ...
      <Container>
        <BannerWrapper>
          ...
          <BannerContentWrapper>
            <BannerContentMain>
              <BannerMainInfo>
                <BannerTitle>Осознанная подборка по Фрилансу</BannerTitle>
              </BannerMainInfo>
              <BannerSubtext>
                Это специальная подборка материалов для старта на фрилансе.
                Её необязательно изучать, чтобы пройти курс. Но если у вас есть
                время, то рекомендую. Это повысит качество вашего обучения
                на курсе.
              </BannerSubtext>
            </BannerContentMain>
            <BannerContentButton>
              <BannerCourseButton
                target="_blank"
                href={"https://granich.design/conscious-freelance-compilation"}
              >
                Изучить
                <CourseArrowOut />
              </BannerCourseButton>
