...
const FreelanceBeforeSvgClip = styled(props => (
  <FreelanceBeforeClip {...props} />
))`
  position: relative;
  display: block;
  width: 100%;
...
const FreelanceBeforeSvgClipMobile = styled(props => (
  <FreelanceBeforeClipMobile {...props} />
))`
  position: relative;
  display: none;
  width: 100%;
...
const FreelanceBefore = () => {
  const data = useStaticQuery(graphql`
    query FreelanceBeforeSectionQuery {
      freelanceBeforeSpider: file(
        relativePath: { eq: "freelance/spider.png" }
      ) {
        childImageSharp {
    ...
    <FreelanceBeforeSection>
      <Container>
        <FreelanceBeforeWrapper>
          <FreelanceBeforeImage
            style={{ position: "absolute" }}
            fluid={spider}
          />
          <FreelanceBeforeInfo>
            <FreelanceBeforeTitle>До Осознанного Фриланса</FreelanceBeforeTitle>
            <FreelanceBeforeDescr>
              Клиенты это странные люди — у них невнятные запросы,
              необоснованные правки, отсутствует чувство прекрасного. А ведь ещё
              и новых клиентов тяжело искать. Ну нафиг, проще в найм пойти.
            </FreelanceBeforeDescr>
          </FreelanceBeforeInfo>
        </FreelanceBeforeWrapper>
