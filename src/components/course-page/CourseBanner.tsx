...
export const PureCourseBanner = ({ box }) => {
  return (
    <BannerSection>
      <Container>
        <BannerWrapper>
          ...
          <BannerContentWrapper>
            <BannerContentMain>
              <BannerMainInfo>
                <BannerTitle>Осознанная подборка по Графдизайну</BannerTitle>
              </BannerMainInfo>
              <BannerSubtext>
                Это специальная подборка материалов для старта в графическом
                дизайне. Её необязательно изучать, чтобы пройти курс. Но если
                у вас есть время, то рекомендую. Это повысит качество вашего
                обучения на курсе.
              </BannerSubtext>
            </BannerContentMain>
            <BannerContentButton>
              <BannerCourseButton
                target="_blank"
                href={
                  "https://granich.design/conscious-graphic-design-compilation"
                }
              >
                Изучить
                <CourseArrowOut />
...
const CourseBanner = () => {
  const imageData = useStaticQuery(graphql`
    query bannerGraphDesignImage {
      graphDesignImageBanner: file(
        relativePath: { eq: "graph-design/course-banner/course-banner.png" }
      ) {
        childImageSharp {
  ...
  return <PureCourseBanner box={box} />
}
