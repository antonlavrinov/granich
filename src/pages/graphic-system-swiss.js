...
import data from "../data/pages/graphic-system-swiss";
...
const GraphicSystemSwissPage = () => {
  const swissOffer = dataIndex.courseCards.edges.find((el) => el.node.courseTitle === "Графсистема Swiss")?.node;
  const swissTeam = dataIndex.team.edges.filter((el) => el.node.teacherName === "Вадим Гранич" || el.node.teacherName === "Андрей Павлушин" || el.node.teacherName === "Елизавета Черникова");
  console.log(swissTeam)
  return (
    <Layout>
      <Header type={"dark"} />
      ...
      />
      <CourseOffer courseName={"Swiss"} data={swissOffer} />
      <CourseExplanations data={data.SwissExplanations} />
      <CourseAuthors
        numberOfTeachers={"3"}
        data={{ edges: swissTeam }}
