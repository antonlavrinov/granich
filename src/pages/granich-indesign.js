...
import ContentCard from "../components/index/precourse-preparation/content-card"
import { ContentCardsWrapper } from "../components/curriculum/styles"
import data from "../data/pages/granich-indesign";
      ...
      ...data?.curriculum?.edges,
      {
        node: {
          ...
          customContent: (<ContentCardsWrapper>
            {dataIndex.contentCardsRecommended.nodes.filter((el) => {
              const condition = el.contentTitle === "Как оформить абзац текста" || el.contentTitle === "Виды гарнитур (шрифтов)" || el.contentTitle === "Как построить сетку. Часть 1";
              return condition;
            }).map((el, idx) => {
              return (
